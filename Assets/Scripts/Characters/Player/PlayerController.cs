...
public class PlayerController : MonoBehaviour, IDamageable
{
    ...
    {
        _rigidbody.linearDamping = falling;
    }
    ...
    public void TakeDamage(int amount, Vector3 direction, Vector3 position)
    {
        ...
        if (_animator != null)
        {
            _sePuedeMover = false;
            _HorizontalMove = 0;
            _rigidbody.linearVelocity = new Vector3(0, _rigidbody.linearVelocity.y, 0); // Detiene movimiento horizontal
    ...
    public void OnMove(float mover)
    {
        if(_sePuedeMover)
        {
            _rigidbody.linearVelocity = new Vector3(mover * _movementVelocity, _rigidbody.linearVelocity.y, 0);
    ...
    public void HoldJumpEhyal(Transform hangPoint)
    {
        ...
        _rigidbody.linearVelocity = Vector3.zero; // Detén cualquier movimiento
    ...
    private void FixedUpdate()
    {
        ...
        if (!_inGround) 
        { 
            float gravityMultiplier = _rigidbody.linearVelocity.y < 0 ? _fallMultiplier : _gravityMultiplier;
